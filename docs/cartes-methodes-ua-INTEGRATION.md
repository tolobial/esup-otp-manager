...
var Home = Vue.extend({
    ...
    // démarre/arrête le défilement quand on bascule fan <-> pairs
    watch: {
        fanMode: function (m) {
    ...
    methods: {
        ...
        fanAutoStart: function () {
            // défile uniquement si vue 'fan' ET mode 'fan' ET pas de survol ET mouvement autorisé
            this.fanAutoStop();
        ...
        fanStartIdle: function () {
            // mouseenter : on stoppe le défilement et on arme le minuteur d'inactivité
            this.fanHover = true;
            this.fanAutoStop();
            this.fanClearIdle();
            if (this.fanMode === 'fan') {
        ...
        fanLeave: function () {
            // mouseleave : on annule le minuteur, on revient à l'éventail et on reprend le défilement
            this.fanHover = false;
            this.fanClearIdle();
...
Les deux sont déjà lus depuis `data()` (`this.fanInterval` dans `fanAutoStart`,
`this.fanIdleDelay` dans `fanStartIdle`) : il suffit de changer le nombre, sans toucher
aux méthodes. Mettre `prefers-reduced-motion` côté OS désactive automatiquement le
défilement (via `fanReducedMotion`).
